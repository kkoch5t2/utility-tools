...
const TOOL_PATH = /^(?:\/schedule\/|\/(image|csv|json|text|pdf|qr|video|japanese|developer|date|calculator)\/)/;
...
test("全ツールをPC・スマホで表示監査する", async ({ browser, request }) => {
  ...
  expect(toolPaths.length).toBeGreaterThanOrEqual(158);
  ...
  for (const viewport of viewports) {
    ...
    for (const path of toolPaths) {
      ...
      if (path === "/schedule/") {
        await expect(page.locator(".schedule-heading h1"), `${viewport.name}: ${path}`).toBeVisible();
